...
static void scofo_read(t_scofo *x, const char *filename)
{
#ifdef PD 
...
static void scofo_free(t_scofo *x)
{
    t_freebytes(x->x_notes, x->x_nnotes * sizeof(t_note));
}
...
void scofo_setup( void)
{
    scofo_class = class_new(gensym("scofo"), (t_newmethod)scofo_new,
    	(t_method)scofo_free, sizeof(t_scofo), 0, 0);
